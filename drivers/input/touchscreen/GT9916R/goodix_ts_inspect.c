...
static int rawdata_test_result = 0;
static int deltadata_result = 0;
static int noisedata_result = 0;
static int short_result = 0;
...
static void goodix_shortcircut_test(struct goodix_ts_test *ts_test)
{
	...
	if (ret < 0) {
		...
		short_result = 1;
		ts_info("short_test pass, short_result = %d\n",short_result);
	}
}
...
static void goodix_capacitance_test(struct goodix_ts_test *ts_test)
{
	...
	if (ret < 0) {
		...
		if (ret < 0) {
			...
			ts_info("rawdata test pass, rawdata_test_result = %d\n", rawdata_test_result);
		}
	}
	...
	if (ret < 0) {
		...
		deltadata_result = 1;
		ts_info("deltadata test pass, deltadata_result = %d\n", deltadata_result);
	}
	...
	if (ts_test->test_params.test_items[GTP_NOISE_TEST]) {
		...
		if (ret < 0) {
			...
			if (ret < 0) {
				...
				noisedata_result = 1;
				ts_info("noisedata test pass, noisedata_result = %d\n", noisedata_result);
			}
		}
	}
...
static int goodix_save_limits(struct goodix_ts_test *ts_test)
{
	...
	if (ts_test->test_result[GTP_CHIP_KEY_INFO_TEST]) {
		...
		bytes += sprintf(&data[bytes], "<ChipInfo Pid=\"%s\" HwVer=\"0x%02x\" PackageSite=\"0x%02x\" FtTestResult=\"0x%02x\" SubstrateTechnology=\"0x%02x\" McuFab=\"0x%02x\">\n",
...
int goodix_openshort_test(struct goodix_ts_core *cd)
{
	int ret;
	struct goodix_ts_test *ts_test = NULL;
